...
queries = catalog_queries.CatalogQueries('sqlite:///db/phonecatalogwithuser.db')
...
def newManufacturerPhone():
    ...
    if 'username' not in login_session:
        print "Not in loggin queries.session?"
	...
	if request.method == 'POST':
		...
		model =  queries.search_by_phone_model_name(new_phone_model)
...
@app.route('/catalog/<manufacturer>/<model>/edit', methods=['GET','POST'])
def editManufacturerPhone(manufacturer,model):
 
    if request.method == 'POST':
    	manufacturer_selected = request.form['manufacturer_list']
        
    	manufacturer = queries.search_by_manufacturer_name(manufacturer_selected)
    	phone_model = queries.session.query(Model).filter_by(name=request.form['name'], manufacturer_id=manufacturer.id).one()
        ...
        flash('%s Changes were Successfully Saved' % phone_model.name)
    	
    	return redirect( url_for('showCatalog') )
    
    else:
    	phone_model = queries.search_by_phone_model_name(model)
    	specs= queries.search_specifications_by_model_id(phone_model.id)
    
        return render_template("editPhoneModel.html",manufacturers=queries.get_phone_models_count(), recent_models=queries.get_recent_models_list(), specs=specs,username=login_session['username'],model=model, manufacturer=manufacturer)

#delete selected phone model    
@app.route('/catalog/<manufacturer>/<model>/delete', methods=['GET','POST'])
def deleteManufacturerPhone(manufacturer, model):
	...
	if request.method == 'POST':
		...
		queries.session.delete(phone_model)
		queries.session.commit()
		
		flash('Phone %s has been successfully deleted' % model.name)
...
@app.route('/catalog/<manufacturer>/<model>')
def showPhoneSpecs(manufacturer, model):

	specifications = queries.session.query(Model, Specifications).join(Specifications, Specifications.model_id==Model.id).filter(Model.name==model).one()[1]
	phone_model = queries.search_by_phone_model_name(model)
	
	if 'username' not in login_session:
		return render_template("phonespecs.html",manufacturers=queries.get_phone_models_count(),  recent_models=queries.get_recent_models_list(), model=model, specs=specifications)
	else:
		if phone_model.user_id == login_session['user_id']:
			return render_template("phonespecs-signedin.html",manufacturers=queries.get_phone_models_count(),  recent_models=queries.get_recent_models_list(), manufacturer=manufacturer, model=model, specs=specifications, username=login_session['username'],authorized=True)
		else:
			flash('You are not authorized to make changes or delete this phone model.')
			return render_template("phonespecs-signedin.html",manufacturers=queries.get_phone_models_count(),  recent_models=queries.get_recent_models_list(), manufacturer=manufacturer, model=model, specs=specifications, username=login_session['username'], authorized=False)
